...
class ProductTemplate(models.Model):
	...
	def write(self, vals):
		if self.is_pack and vals.get('wk_product_pack'):
			pack_lines_obj =[]
			for pack_line in vals.get('wk_product_pack'):
			...
			if vals.get('wk_product_pack') and not pack_lines_obj:
				raise UserError(
					'You can not create a pack without products!!!')
		if not vals.get('is_pack'):
